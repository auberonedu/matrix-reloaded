...
public class MazeSolver {
    ...
    public static boolean reachable(int row, int col, int[][] maze) {
        ...
        if(maze[row][col] == 1){
            throw new IllegalArgumentException("Location is in a wall: " + row + "," + col);
        }
    ...
    private static List<Location> solve(int row, int col, int[][] maze, boolean[][] visited) {
        ...
        for (int[] neighbor : neighbors) {
            ...
            List<Location> subPath = solve(newRow, newCol, maze, visited);
            if (subPath != null) {
                subPath.add(0, new Location(row, col));
                return subPath;
            }
        }
        return null;
